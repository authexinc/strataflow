...
| Product URLs | **`/app/<path>` in the web client; nginx 301s every `/odoo` URL to `/app` at the edge** | Answered 2026-09-10, superseding the 2026-09-08 row. The web client is served at `/app` as well as `/odoo` (`controllers/home.py` repeats stock's route list plus `/app`), and the client's router is taught the prefix in `static/src/core/app_url.js`: `router.stateToUrl` / `router.urlToState` are on the exported object stock marks as patchable, and `startRouter()` is called again after the patch because stock ran it at import. What the 2026-09-08 attempt hit — the router's internal-link guard, which only recognises `/odoo` — is left alone: an `<a href="/odoo/…">` click is a full page load (served directly in dev, 301'd by nginx in prod), not a broken one. Odoo's own paths keep saying `/odoo` server-side (login redirect, logout, the webmanifest); `deploy/nginx/flow.strataline.co` rewrites them (`rewrite … permanent` on `/odoo`, `proxy_redirect` on Location headers) so a browser never shows the vendor. `path` on each client action plus `static path` on each screen component is still what makes `/app/<path>` resolve; keep both. **A path may not equal any stock client-action tag**: the web client resolves a URL by registry tag before path, and stock's `home` tag is an action that navigates to `/` — so the Home screen is `/app/desk`, not `/app/home`. Check new paths against `registry.category("actions").add("…")` across `addons/*/static/src`. |
...
| Stock views (forms, lists, kanban, dialogs, Settings) | **Strataline language, tenant-wide, light only** | Answered 2026-09-09. No per-user gating: tenancy is DB-per-tenant, so every internal user is the tenant's and there is nobody to keep the stock look for. Built through Odoo's variable seams (`static/scss/backend_variables.scss` prepended to `web._assets_primary_variables`, `backend_bootstrap.scss` to `web._assets_backend_helpers`) plus one material file, `static/src/stock/stock.scss`: glass on the functional layer, solid surfaces for content, never a bare `.o_web_client` rule beyond the token block. Light only because Odoo CE serves one colour scheme (`ir.http.color_scheme()` is hard-coded); the shell's dark toggle stays with the shell. |
| Primary buttons | **Accent, everywhere** | Answered 2026-09-09. Ink means "you are here" (selected nav, active chip), accent means "this is the action". `.o_sf_btn--primary` and the stock `.btn-primary` both fill with `--accent`; the ink mixin is left to selection states. |
| Ticket-side postal code for the zone match | **A `postal_code` field on `strataflow.workorder`**, defaulted from the requester's `zip` only when empty | Answered 2026-09-09. The dig site is not the requester's billing address — a contractor's HQ is in one zone and their sites in many — so deriving from `res.partner.zip` would mis-zone exactly the tickets auto-assign exists for. "+ New ticket" and the USP feed populate it. Not built — see `BACKLOG.md` › Behaviour. |
| Map layers | **Strataline's layer panel, rebuilt from its manifest** | Built 2026-09-09 (`core/layers.js`, `core/layer_panel.js`). Rows come from strataline's `layers.json` (one per dataset inside 32 merged style layers, told apart by the `sub_layer` attribute), plus this client's own basemap and reference layers. Presets, filter, tri-state group toggles, safety layers that stay on. Only the user's overrides persist, on the device. Hidden datasets become a `!in sub_layer` filter clause composed with the layer's baked filter. Privacy exclusions mirror strataline's client list until `layers.json` carries the flag. Company filter (`owners.json`) not ported. |
| Locate drawings | **Geo-referenced: `[lng, lat]` per line end and note, drawn as a layer on the live map** | Answered 2026-09-09 (Stefan). The print pans and zooms with the utilities it marks and its metres are real (haversine). Format in `core/locate_geo.js` (`{v: 2, segments: [{a, b, util}], notes: [{at, text}]}`); the pixel format is treated as empty — demo data never had drawings. Preview, PNG, CSV and PDF re-project north-up with a printed scale (`controllers/export.py` mirrors `projectDrawing`). The alternative — pixels over a frozen map — was rejected: the locator could not move the map while drawing. |
| Live map | **MapLibre in the web client, Strataline over its API with the tenant's key as `?key=`** | Built 2026-09-09 (`core/strataline_map.js`). The key goes to the browser on purpose (Phase 1 above); strataline echoes CORS only for the key's registered origins and has no preflight handler, so a header would fail. Strataline supplies basemap + utility tiles, `/style.json`, glyphs and `/tiles/meta`; the ground palette is Strataflow's own (from the shell tokens, both themes) and utility layers keep the industry colours. MapLibre 5.24.0 is vendored in `static/lib` and loaded lazily — 6.x is ESM-only, which Odoo's pipeline cannot bundle. Satellite is Esri World Imagery, the source strataline itself uses. Nothing from `map-sys` is copied; the style is fetched at runtime. |
